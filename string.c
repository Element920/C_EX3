...
int similiar(char* t , char* s , int n){
    int lent = strlen(t) , lens = strlen(s);
    if(isEqualString(s,t,lent)){
        return 1;
    }
    char temp[lens];
    if(lent-lens!=n){
